...
//					System.out.println("Ne moremo priti do najblizje refinerije!");
	...
	public boolean runBaseBuild() throws GameActionException {
		...
		if (fazaBaze == 0) {
			...
			if (rc.canSenseLocation(f1) && rc.senseRobotAtLocation(f1) != null
					&& rc.senseRobotAtLocation(f1).type == RobotType.FULFILLMENT_CENTER
					&& rc.getTeamSoup() >= RobotType.DESIGN_SCHOOL.cost + konst.desing_school_buffer) {
				...
				if (rc.getLocation().distanceSquaredTo(f1) <= 2 && !f1.equals(rc.getLocation())) {
					if (rc.getTeamSoup() >= RobotType.FULFILLMENT_CENTER.cost + konst.fullfilemnt_buffer
							&& rc.canBuildRobot(RobotType.FULFILLMENT_CENTER, rc.getLocation().directionTo(f1))) {
		...
		if (positioned) {
			if (dist == 2) {
				if (rc.getDirtCarrying() > 0) {
					for (Direction d : Direction.allDirections()) {
						if (Util.d_inf(hq, rc.getLocation().add(d)) == 1) {
					...
					for (Direction d : Util.dir) {
						if (Util.d_inf(rc.getLocation().add(d), hq) == 3) {
							rc.digDirt(d);
							return;
						}
					}
